...
imgx = 5000; imgy = 5000 # image size
...
beta = 0.7
gamma = 0.001
mx = 100; my = 100 # width and height of 2DCA
...
ca[int(mx/2)][int(my/2)] = 1.0 # ice seed
...
for ky in range(imgy):
    for kx in range(imgx):
        ...
        if tx >= 0 and tx <= imgx1 and ty >= 0 and ty <= imgy1:
            ...
            if c >= 1.0: # ice
                c = int((c - 1.0) * 255)
                pixels[kx, ky] = (int(c % mr0 * mr1), int(c % mr0 * mr1), int(c % mr0 * mr1))
